...
function PrintGitLatestLog(botClient, message) {
  ...
  message.channel.send(`Running ${(botClient.config.githubLogUrl) ? `[commit](${botClient.config.githubLogUrl + githash.log.substr(0, 7)})` : "commit"}${(!githash.workingClean) ? " __*w/changes*__" : ""} = \`${githash.log}\``);
}